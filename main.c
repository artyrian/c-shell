...
/*
void WordAmp (struct checkers *flg)
{
	flg->fWordOut = !OFF;
	if ( !(flg->fBg) ){
		flg->fBg = !OFF;
		...
		flg->fError = !OFF;
		printf ("Error of '&'. Check number!");
	}
}


void WordStick (struct checkers *flg)
{
	flg->fWordOut = !OFF;
	if ( !(flg->fstick) ){
		flg->fstick = !OFF;
...
void WordRead(struct checkers *flg, struct frame *p)
{
	flg->fWordOut = !OFF;
	if ( !(flg->fRead) ){
		flg->fRead = !OFF;
...
void WordWrite(struct checkers *flg, struct frame *p)
{
	flg->fWordOut = !OFF;
	if ( !(flg->fWrite) ){
		flg->fWrite = !OFF;
...
void WordAppend(struct checkers *flg, struct frame *p)
{
	flg->fWordOut = !OFF;
	if ( !(flg->fAppend) ){
		flg->fAppend = !OFF;
